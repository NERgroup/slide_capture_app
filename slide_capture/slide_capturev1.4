...
class SlideCaptureApp(QMainWindow):
    GRID_ROWS = 10
    GRID_COLS = 10
    MAX_REPLICATES = GRID_ROWS * GRID_COLS
    ...
    def __init__(self) -> None:
        ...
        self.grid_coords = self.build_upc_grid()
    ...
    def build_upc_grid(self) -> list[tuple[int, int]]:
        xs = self.build_axis(self.X_START, self.X_END, self.GRID_COLS)
        ys = self.build_axis(self.Y_START, self.Y_END, self.GRID_ROWS)
    ...
    def _build_ui(self) -> None:
        ...
        self.subtitle_label = QLabel(
            "Single calibration image per site • 100 replicate photos per slide • 10×10 snake grid"
        )
        ...
        self.replicate_spin.setRange(1, self.MAX_REPLICATES)
        ...
        slide_layout.addWidget(QLabel("Replicate photo"), 2, 0)
        slide_layout.addWidget(self.replicate_spin, 2, 1)
        slide_layout.addWidget(self.auto_increment_checkbox, 3, 0, 1, 2)
        slide_layout.addWidget(self.prev_replicate_btn, 4, 0)
        slide_layout.addWidget(self.next_replicate_btn, 4, 1)
        ...
        self.replicate_indicator = self._make_indicator_value(f"1 / {self.MAX_REPLICATES}")
    ...
    def refresh_summary(self) -> None:
        ...
        text = (
            ...
            f"Grid:\n"
            f"• 10 × 10 snake pattern\n"
            f"• Start: {self.X_START}x{self.Y_START}\n"
            f"• End: {self.X_END}x{self.Y_END}\n"
            ...
            f"• Capture 100 replicate photos per slide\n"
