...
EXTRA_NAME_PATTERN = re.compile(r"[^A-Za-z0-9_-]+")
...
class SlideCaptureApp(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.pending_extra_description = ""
    ...
    def _build_ui(self) -> None:
        ...
        self.capture_extra_btn = QPushButton("CAPTURE EXTRA / COOL FIND")
        self.capture_extra_btn.setMinimumHeight(50)
        self.capture_extra_btn.setObjectName("ExtraButton")
        ...
        capture_layout.addWidget(self.capture_extra_btn)
    ...
    def _connect_signals(self) -> None:
        ...
        self.capture_extra_btn.clicked.connect(self.capture_extra_image)
    ...
    def extra_folder(self) -> Path:
        return self.output_root / "EXTRA_cool_finds"

    def clean_extra_description(self, description: str) -> str:
        cleaned = description.strip().lower()
        cleaned = EXTRA_NAME_PATTERN.sub("_", cleaned)
        cleaned = cleaned.strip("_")
        return cleaned[:80]

    def build_extra_filename(self, state: CaptureState, description_slug: str) -> str:
        system_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        return f"{system_stamp}_{state.site_code}_EXTRA_{description_slug}.png"
    ...
    def set_busy_ui(self, busy: bool) -> None:
        ...
        self.capture_extra_btn.setEnabled(not busy)
    ...
    def refresh_summary(self) -> None:
        ...
        text = (
            ...
            f"Extra/cool-find images folder:\n{self.extra_folder()}\n\n"
            ...
            f"• Replicate and calibration images save directly into the site folder\n"
            f"• Extra/cool-find images save separately and require a descriptive name"
        )
    ...
    def capture_extra_image(self) -> None:
        if self.capture_in_progress:
            return
        ...
        description, ok = QInputDialog.getText(
            self,
            "Name extra/cool-find image",
            "Brief descriptive name for this photo, e.g., snail_eggs_on_slide or odd_red_algae_fragment:",
        )
        if not ok:
            return

        description_slug = self.clean_extra_description(description)
        if not description_slug:
            QMessageBox.warning(
                self,
                "Description needed",
                "Please enter a short descriptive name using letters, numbers, spaces, underscores, or hyphens.",
            )
            return

        folder = self.extra_folder()
        full_path = folder / self.build_extra_filename(state, description_slug)

        if full_path.exists():
            answer = QMessageBox.question(
                self,
                "Extra image already exists",
                f"This extra image already exists:\n\n{full_path.name}\n\nReplace it?",
                QMessageBox.Yes | QMessageBox.No,
                QMessageBox.No,
            )
            if answer != QMessageBox.Yes:
        ...
        self.pending_extra_description = description.strip()
        self.begin_capture(
            full_path,
            state,
            "extra",
            f"Capturing extra/cool-find image {full_path.name}...",
        )
    ...
    def capture_done(self, job) -> None:
        try:
            ...
            elif self.pending_capture_kind == "extra":
                self.write_extra_log(
                    self.pending_file_path,
                    self.pending_state,
                    self.pending_extra_description,
                )
                self.statusBar().showMessage(f"Captured extra/cool-find image: {self.pending_file_path.name}")
    ...
    def write_extra_log(self, path: Path, state: CaptureState, description: str) -> None:
        log_file = path.parent / "extra_log.csv"
        exists = log_file.exists()
        with open(log_file, "a", newline="", encoding="utf-8") as handle:
            writer = csv.writer(handle)
            if not exists:
